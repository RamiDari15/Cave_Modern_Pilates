...
def is_public_service_name(name: str) -> bool:
    lowered = re.sub(r"\s+", " ", name.lower()).strip()

    if not lowered:
        return False

    if any(term in lowered for term in ["cave1", "test", "training"]):
        return False

    if any(term in lowered for term in ["new client", "newbie", "starter", "intro"]):
        return True
...
def normalize_store(results: Dict[str, Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    services = [
        ...
        and is_sell_online(item)
        and not item.get("Discontinued")
        and is_public_service_name(item_name(item))
    ]
    contracts = [
        ...
        normalized.append(
            {
                ...
                "bookUrl": f"/schedule?classId={item.get('Id')}" if item.get("Id") else "/schedule",
            }
        )
